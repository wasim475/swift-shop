...
const Navbar = () => {
  ...
  const totalPrice = cartInfo.reduce((sum, product) => sum + product.price, 0);
      ...
      <nav className="bg-gray-800 flex justify-between">
        <div>
          <p className="text-gray-400 text-sm flex items-center">
            {" "}
            <IoLocationOutline /> store location: Dhamrai, Dhaka, Bangladesh.
          </p>
        </div>
        <div className="text-md text-gray-400 flex gap-x-5">
          {currentUser ? (
            <ProfileDropdown setUser={setCurrentUser} user={currentUser} />
          ) : (
            <>
          ...
          {currentUser?.role === "admin" && (
            <Link href={'/dashboard'} className="flex items-center gap-2 px-4 bg-red-700 font-bold text-white rounded-lg shadow-md hover:bg-white hover:text-red-700 transition-all">
              Admin Pannel
            </Link>
          )}
        </div>
